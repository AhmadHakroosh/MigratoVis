...
(function (scope) {
	scope.layout = function() {
		...
		function relayout () {
			...
			data = data || {mapping: {}, names: [], regions: []};
			year = year || Object.keys(data.mapping)[0];
			mapping = year && data.mapping[year] || [];
			...
			k = 0, i = -1; while (++i < n) {
				x = 0, j = -1; while (++j < n) {
					x += mapping[indices[i]][indices[j]];
					x += mapping[indices[j]][indices[i]];
				}
				groupSums.push(x);
			...
			if (sortSubgroups) {
				subgroupIndex.forEach(function (d, i) {
					d.source.sort(function (a, b) {
						return sortSubgroups(mapping[indices[i]][indices[a]], mapping[indices[i]][indices[b]]);
					});
					d.target.sort(function (a, b) {
						return sortSubgroups(mapping[indices[a]][indices[i]], mapping[indices[b]][indices[i]]);
					});
				});
			}
			...
			x = chord.alpha(), i = -1; while (++i < n) {
				...
				x0 = x, j = -1; while (++j < n) {
					...
					v = mapping[indices[dj]][indices[di]],
					a0 = x,
					d = v * k;
					x += d;
				...
				x0 = x, j = -1; while (++j < n) {
					var dj = subgroupIndex[di].source[j],
						v = mapping[indices[di]][indices[dj]],
						a0 = x,
						d = v * k;
						x += d;
		...
		chord.countries = function (x) {
			...
			countries = x;
			indices = scope.merge(data, countries);
			n = indices.length;
			chords = groups = null;
			return chord;
		};
...
(function (scope) {
	scope.chart = function (data, config) {
		...
		var chordGenerator = migrato.chord()
